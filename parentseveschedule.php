...
/// Print the page header
    ...
    print_header_simple(get_string('parentseveschedule','parentseve'), '', $navigation, "", "", true,
                  '');

    //Display tabs
    termreview_printtabs($cmid, $termreview, $parentreview, 3);
    if (!$CFG->termreview_enableparentseves) {
        print_error('parentsevesdisabled','termreview');
    } elseif(!$bookappointments  && empty($parentname) and has_capability('mod/termreview:viewallparentseve', $context)) {
        //show all teachers' schedules
        add_to_log(0, "termreview", "View schedule", "parentseveschedule.php?id=$cmid", "$cmid");

        echo '<a href="'.$CFG->wwwroot.'/mod/termreview/parentseveschedule.php?book=1&cmid='.$cmid.'">'.get_string('bookapps','termreview').'</a>';

        $teachers=termreview_get_parentseve_teachers($parentseve->id);
        foreach($teachers as $teacher){
            termreview_print_pe_schedule($teacher,$parentseve);
        }
    //is this user on the list of users who can termreview:write in any context?
    } elseif (!$bookappointments  && empty($parentname) and termreview_parentseve_isteacher($USER->id,$parentseve->id)) {
        //show current users schedule
        add_to_log(0, "termreview", "View schedule", "parentseveschedule.php?id=$cmid", "$cmid");

        echo '<p><a href="'.$CFG->wwwroot.'/mod/termreview/parentseveschedule.php?book=1&cmid='.$cmid.'">'.get_string('bookapps','termreview').'</a></p>';

        if(!termreview_print_pe_schedule($USER,$parentseve)){
            print_string('emptyschedule','termreview',$USER->firstname.' '.$USER->lastname);
        }
    } else {
    ...
    add_to_log(0, "termreview", "View schedule", "parentseveschedule.php?id=$cmid", "$cmid");
        ...
        if(empty($parentname)) {
            ...
            echo get_string('parentseveon','termreview',array('date'=>date('l jS F Y',$parentseve->timestart))).'<p class="termreview_pe_info">'.$parentseve->info.'</p>'.
                ...
                <form method="post" action="parentseveschedule.php" id="termreview_pe_form" onSubmit="return termreview_pe_validate(this)" ><input name="cmid" type="hidden" value="'.$cmid.'"><div id="termreview_pe_names">'.get_string('parentname','termreview').'<input type="text" name="parentname">'.get_string('studentname','termreview').'<input type="text" name="studentname"></div>
                <div id="termreview_pe_buttons"><button type="button" onClick="newAppointment()">'.get_string('newapp','termreview').'</button>
                <input type="submit" value="'.get_string('confirmapps','termreview').'"></div><div id="termreview_pe_appointments"><!--AJAX will put the schedules in here--></div><div style="clear:both;"></div></form>';
        } else {
            $success = 0;
            $fail = 0;
            ...
            foreach ($newappointments as $key => $newappointment){
                if($teacher = get_record('user','id',$newappointmentteachers[$key])){
                    ...
                    if(get_record('termreview_parentseve_app','teacherid',$appointment->teacherid,'apptime',$appointment->apptime)==false and insert_record('termreview_parentseve_app',$appointment)){
                        echo get_string('appbooked','termreview',array('teacher'=>$teacher->firstname.' '.$teacher->lastname,'apptime'=>date('G:i',$appointment->apptime))).'<br>';
                        $success++;
                    }else{
                        echo get_string('appnotbooked','termreview',array('teacher'=>$teacher->firstname.' '.$teacher->lastname,'apptime'=>date('G:i',$appointment->apptime))).'<br>';
                        $fail++;
                    }
                }
            }
            echo '<br>'.get_string('success','termreview',$success);
            if($fail>0)echo '<br>'.get_string('fail','termreview',$fail);
            echo'<h4>'.get_string('printsave','termreview').'</h4>';
            echo'<p><a href="'.$CFG->wwwroot.'/mod/termreview/parentseveschedule.php?cmid='.$cmid.'">'.get_string('backtoappointments', 'termreview').'</a></p>';
        }
    }
